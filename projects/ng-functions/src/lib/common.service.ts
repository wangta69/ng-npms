...
export class CommonService {
   ...
   */
  remove_item(value: string, haystack: []) {
    return haystack.filter(v => v !== value);
  }

  array_search(needle: string, haystack: []) {
    for (const i in haystack) {
      if (haystack[i] === needle) {
        return i;
      }
  ...
  arrayPushUnique(needle: string, haystack: string[], size ?: number ) {
  ...
  hasOwnProp(obj: any, propName: string) {
    //    return obj.hasOwnProperty(propName);
    return Object.prototype.hasOwnProperty.call(obj, propName);
  }

  getOwnProperty(obj: any, propName: string) {
